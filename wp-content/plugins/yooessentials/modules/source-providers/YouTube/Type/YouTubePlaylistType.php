...
class YouTubePlaylistType extends GenericType
{
    public const NAME = 'YouTubePlaylistType';
    ...
    public function config(): array
    {
        return [
            ...
            'fields' => [
                ...
                'videos' => [
                    'type' => ['listOf' => YouTubeVideoType::NAME],
                    'metadata' => [
                        'label' => 'Videos',
                    ],
                ],
    ...
    public static function resolveProp(array $playlist, array $args)
    {
        return Arr::get($playlist, $args['path']);
    }

    public static function resolveIframe(array $playlist, array $args)
    {
    ...
    public static function resolveThumbnailUrl(array $playlist, array $args): string
    {
        $id = $playlist['id'] ?? '';
        $size = $args['size'] ?? self::DEFAULT_THUMB_SIZE;
        $url = $playlist['snippet']['thumbnails'][$size]['url'] ?? '';

        if ($id && $url) {
            return Util\File::cacheMedia($url, "youtube-playlist-thumb-{$id}-{$size}");
        }

        return '';
    }

    public static function resolveThumbnailWidth(array $playlist, array $args): int
    {
        $size = $args['size'] ?? self::DEFAULT_THUMB_SIZE;

        return $playlist['snippet']['thumbnails'][$size]['width'] ?? 0;
    }

    public static function resolveThumbnailHeight(array $playlist, array $args): int
    {
        $size = $args['size'] ?? self::DEFAULT_THUMB_SIZE;

        return $playlist['snippet']['thumbnails'][$size]['height'] ?? 0;
    }
}

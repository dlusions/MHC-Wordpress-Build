...
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Aggregate\PredisCluster;
...
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\CacheItem;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\LogicException;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Traits\RedisClusterProxy;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Traits\RedisProxy;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()

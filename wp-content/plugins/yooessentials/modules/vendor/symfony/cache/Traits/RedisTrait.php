 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Traits;

use ZOOlanders\YOOessentials\Vendor\Predis\Command\Redis\UNLINK;
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Aggregate\ClusterInterface;
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Aggregate\RedisCluster;
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Aggregate\ReplicationInterface;
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use ZOOlanders\YOOessentials\Vendor\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
use ZOOlanders\YOOessentials\Vendor\Predis\Response\ErrorInterface;
use ZOOlanders\YOOessentials\Vendor\Predis\Response\Status;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
        ...
        if ($redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\ZOOlanders\YOOessentials\Vendor\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\ZOOlanders\YOOessentials\Vendor\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \ZOOlanders\YOOessentials\Vendor\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !\is_a($class, \ZOOlanders\YOOessentials\Vendor\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
        ...
        } elseif (\is_a($class, \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
    ...
    {
        if ($this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ZOOlanders\YOOessentials\Vendor\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, ?object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \ZOOlanders\YOOessentials\Vendor\Predis\ClientInterface) {
            ...
            if (($connection instanceof ClusterInterface || $connection instanceof Predis2ClusterInterface) && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \ZOOlanders\YOOessentials\Vendor\Predis\Client($c);
                }
            }

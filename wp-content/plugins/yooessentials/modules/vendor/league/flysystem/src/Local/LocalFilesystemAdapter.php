...
namespace ZOOlanders\YOOessentials\Vendor\League\Flysystem\Local;
...
use ZOOlanders\YOOessentials\Vendor\League\Flysystem\SymbolicLinkEncountered;
...
class LocalFilesystemAdapter implements FilesystemAdapter
{
    ...
    private function writeToFile(string $path, $contents, Config $config) : void
    {
        ...
        $this->ensureDirectoryExists(dirname($prefixedLocation), $this->resolveDirectoryVisibility($config->get(Config::OPTION_DIRECTORY_VISIBILITY)));
        error_clear_last();
    ...
    {
        yield from new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS), $mode);
    }
    ...
    public function listContents(string $path, bool $deep) : iterable
    {
        ...
        foreach ($iterator as $fileInfo) {
            ...
            (yield $isDirectory ? new DirectoryAttributes($path, $visibility, $lastModified) : new FileAttributes(\str_replace('\\', '/', $path), $fileInfo->getSize(), $visibility, $lastModified));
        }
    }
    public function move(string $source, string $destination, Config $config) : void
    {
        ...
        $this->ensureDirectoryExists(dirname($destinationPath), $this->resolveDirectoryVisibility($config->get(Config::OPTION_DIRECTORY_VISIBILITY)));
    ...
    protected function ensureDirectoryExists(string $dirname, int $visibility) : void
    {
        ...
        error_clear_last();
        if (!@mkdir($dirname, $visibility, \true)) {
            $mkdirError = error_get_last();
        }
        clearstatcache(\true, $dirname);
        if (!is_dir($dirname)) {
    ...
    public function createDirectory(string $path, Config $config) : void
    {
        ...
        error_clear_last();
        if (!@mkdir($location, $permissions, \true)) {
    ...
    public function setVisibility(string $path, string $visibility) : void
    {
        ...
        $visibility = is_dir($path) ? $this->visibility->forDirectory($visibility) : $this->visibility->forFile($visibility);
    ...
    {
        return $visibility === null ? $this->visibility->defaultForDirectories() : $this->visibility->forDirectory($visibility);
    }

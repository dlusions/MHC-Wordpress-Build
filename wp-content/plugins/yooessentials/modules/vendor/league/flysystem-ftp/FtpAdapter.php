...
class FtpAdapter implements FilesystemAdapter
{
    ...
    public function __construct(FtpConnectionOptions $connectionOptions, FtpConnectionProvider $connectionProvider = null, ConnectivityChecker $connectivityChecker = null, VisibilityConverter $visibilityConverter = null, MimeTypeDetector $mimeTypeDetector = null)
    {
    ...
    private function fetchMetadata(string $path, string $type) : FileAttributes
    {
        ...
        if (!$attributes instanceof FileAttributes) {
            throw UnableToRetrieveMetadata::create($path, $type, 'expected file, ' . ($attributes instanceof DirectoryAttributes ? 'directory found' : 'nothing found'));
        }
        return $attributes;
    }
    ...
    private function normalizeUnixObject(string $item, string $base) : StorageAttributes
    {
        ...
        if ($isDirectory) {
            return new DirectoryAttributes($path, $this->visibilityConverter->inverseForDirectory($permissions), $lastModified);
        }

...
use ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Sts\ValueObject\ProvidedContext;
...
class StsClient extends AbstractApi
{
    ...
    public function assumeRole($input) : AssumeRoleResponse
    {
        ...
        $response = $this->getResponse($input->request(), new RequestContext(['operation' => 'AssumeRole', 'region' => $input->getRegion(), 'exceptionMapping' => ['ExpiredTokenException' => ExpiredTokenException::class, 'MalformedPolicyDocument' => MalformedPolicyDocumentException::class, 'PackedPolicyTooLarge' => PackedPolicyTooLargeException::class, 'RegionDisabledException' => RegionDisabledException::class]]));
    ...
    public function assumeRoleWithWebIdentity($input) : AssumeRoleWithWebIdentityResponse
    {
        ...
        $response = $this->getResponse($input->request(), new RequestContext(['operation' => 'AssumeRoleWithWebIdentity', 'region' => $input->getRegion(), 'exceptionMapping' => ['ExpiredTokenException' => ExpiredTokenException::class, 'IDPCommunicationError' => IDPCommunicationErrorException::class, 'IDPRejectedClaim' => IDPRejectedClaimException::class, 'InvalidIdentityToken' => InvalidIdentityTokenException::class, 'MalformedPolicyDocument' => MalformedPolicyDocumentException::class, 'PackedPolicyTooLarge' => PackedPolicyTooLargeException::class, 'RegionDisabledException' => RegionDisabledException::class]]));
    ...
    protected function getEndpointMetadata(?string $region) : array
    {
        ...
        switch ($region) {
            ...
            case 'us-isof-south-1':
                return ['endpoint' => "https://sts.{$region}.csp.hci.ic.gov", 'signRegion' => $region, 'signService' => 'sts', 'signVersions' => ['v4']];
            case 'eu-isoe-west-1':
                return ['endpoint' => 'https://sts.eu-isoe-west-1.cloud.adc-e.uk', 'signRegion' => 'eu-isoe-west-1', 'signService' => 'sts', 'signVersions' => ['v4']];
            case 'us-isob-east-1':

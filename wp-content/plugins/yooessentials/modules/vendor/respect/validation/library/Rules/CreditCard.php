...
final class CreditCard extends AbstractRule
{
    ...
    private const BRAND_REGEX_LIST = [self::ANY => '/^[0-9]+$/', self::AMERICAN_EXPRESS => '/^3[47]\\d{13}$/', self::DINERS_CLUB => '/^3(?:0[0-5]|[68]\\d)\\d{11}$/', self::DISCOVER => '/^6(?:011|5\\d{2})\\d{12}$/', self::JCB => '/^(?:2131|1800|35\\d{3})\\d{11}$/', self::MASTERCARD => '/(5[1-5]|2[2-7])\\d{14}$/', self::VISA => '/^4\\d{12}(?:\\d{3})?$/', self::RUPAY => '/^6(?!011)(?:0[0-9]{14}|52[12][0-9]{12})$/'];
    /**
     * @var string
     */
    private $brand;
    ...
    public function __construct(string $brand = self::ANY)
    {
        if (!isset(self::BRAND_REGEX_LIST[$brand])) {
            throw new ComponentException(sprintf('"%s" is not a valid credit card brand (Available: %s)', $brand, implode(', ', array_keys(self::BRAND_REGEX_LIST))));
        }
        $this->brand = $brand;
    }
    /**

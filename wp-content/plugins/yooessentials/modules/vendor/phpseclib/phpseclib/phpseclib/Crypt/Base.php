...
abstract class Base
{
    ...
    function __construct($mode = self::MODE_CBC)
    {
        ...
        if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_BASE_USE_REG_INTVAL')) {
            switch (\true) {
                ...
                case \PHP_INT_SIZE == 8:
                    \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_BASE_USE_REG_INTVAL', \true);
                    break;
                case (\php_uname('m') & "\xdf\xdf\xdf") == 'ARM':
                    switch (\true) {
                        ...
                        case \PHP_VERSION_ID >= 70200 && \PHP_VERSION_ID <= 70211:
                            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_BASE_USE_REG_INTVAL', \false);
                            break;
                        default:
                            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_BASE_USE_REG_INTVAL', \true);
                    }
    ...
    function decrypt($ciphertext)
    {
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            ...
            switch ($this->mode) {
                ...
                case self::MODE_ECB:
                    if (!\defined('OPENSSL_RAW_DATA')) {
                        $ciphertext .= @\openssl_encrypt('', $this->cipher_name_openssl_ecb, $this->key, \true);
                    }
                    $plaintext = \openssl_decrypt($ciphertext, $this->cipher_name_openssl, $this->key, $this->openssl_options);
                    break;
                case self::MODE_CBC:
                    if (!\defined('OPENSSL_RAW_DATA')) {
                        ...
                        $ciphertext .= \substr(@\openssl_encrypt($padding, $this->cipher_name_openssl_ecb, $this->key, \true), 0, $this->block_size);
    ...
    {
        $this->engine = null;
        $candidateEngines = array($this->preferredEngine, self::ENGINE_OPENSSL, self::ENGINE_MCRYPT);
        ...
        switch (\true) {
            case self::$WHIRLPOOL_AVAILABLE:
                foreach (\str_split($bytes, 64) as $t) {
                    $hash = \hash('whirlpool', $hash, \true);
                    $result .= $t ^ $hash;
                }
                return $result . \hash('whirlpool', $hash, \true);
            default:
                $len = \strlen($bytes);
                for ($i = 0; $i < $len; $i += 20) {
                    $t = \substr($bytes, $i, 20);

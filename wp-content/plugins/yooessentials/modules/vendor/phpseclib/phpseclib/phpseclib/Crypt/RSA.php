 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\phpseclib\Crypt;
...
class RSA
{
    ...
    function __construct()
    {
        ...
        if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE')) {
            switch (\true) {
                // Math/BigInteger's openssl requirements are a little less stringent than Crypt/RSA's. in particular,
                // Math/BigInteger doesn't require an openssl.cfg file whereas Crypt/RSA does. so if Math/BigInteger
                // can't use OpenSSL it can be pretty trivially assumed, then, that Crypt/RSA can't either.
                case \defined('ZOOlanders\\YOOessentials\\Vendor\\MATH_BIGINTEGER_OPENSSL_DISABLE'):
                    \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE', self::MODE_INTERNAL);
                    break;
                case \function_exists('phpinfo') && \extension_loaded('openssl') && \file_exists($this->configFile):
                    ...
                    switch (\true) {
                        ...
                        case \version_compare($versions['Header'], '1.0.0') >= 0 && \version_compare($versions['Library'], '1.0.0') >= 0:
                            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE', self::MODE_OPENSSL);
                            break;
                        default:
                            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE', self::MODE_INTERNAL);
                            \define('ZOOlanders\\YOOessentials\\Vendor\\MATH_BIGINTEGER_OPENSSL_DISABLE', \true);
                    }
                    break;
                default:
                    \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE', self::MODE_INTERNAL);
    ...
    {
        if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_EXPONENT')) {
            // http://en.wikipedia.org/wiki/65537_%28number%29
            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_EXPONENT', '65537');
        }
        ...
        if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_SMALLEST_PRIME')) {
            \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_SMALLEST_PRIME', 4096);
        }
    ...
    function _exponentiate($x)
    {
        ...
        if (\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_DISABLE_BLINDING')) {
    ...
    function _rsaes_pkcs1_v1_5_encrypt($m)
    {
        ...
        if (\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_PKCS15_COMPAT') && (!isset($this->publicExponent) || $this->exponent !== $this->publicExponent)) {
            $type = 1;

...
class Blowfish extends Base
{
    ...
    function _expand0state($key, &$sbox0, &$sbox1, &$sbox2, &$sbox3, &$p)
    {
        ...
        $p = array($p[0] ^ $key[0], $p[1] ^ $key[1], $p[2] ^ $key[2], $p[3] ^ $key[3], $p[4] ^ $key[4], $p[5] ^ $key[5], $p[6] ^ $key[6], $p[7] ^ $key[7], $p[8] ^ $key[8], $p[9] ^ $key[9], $p[10] ^ $key[10], $p[11] ^ $key[11], $p[12] ^ $key[12], $p[13] ^ $key[13], $p[14] ^ $key[14], $p[15] ^ $key[15], $p[16] ^ $key[0], $p[17] ^ $key[1]);
    ...
    function _encryptBlock($in)
    {
        ...
        $l = $in[1];
        $r = $in[2];
        list($r, $l) = \PHP_INT_SIZE === 8 ? $this->_encryptBlockHelperFast($l, $r, $sb_0, $sb_1, $sb_2, $sb_3, $p) : $this->_encryptBlockHelperSlow($l, $r, $sb_0, $sb_1, $sb_2, $sb_3, $p);
        return \pack("N*", $r, $l);
    }
    /**
